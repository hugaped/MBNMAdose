...
print.nma.nodesplit <- function(nodesplit) {
  ...
  comparisons <- names(nodesplit)
  colnam <- "comparison\tp.value\t\t\tMedian (95% CrI)"
  paramsect <- colnam
  for (i in seq_along(comparisons)) {
    pval <- signif(nodesplit[[i]]$p.values,
                   max(3L, getOption("digits") - 3L))
    tab <- nodesplit[[i]]$quantiles

    heading <- paste(names(nodesplit)[i], pval, sep=width)
...
summary.nma.nodesplit <- function(nodesplit) {
  ...
  if ("quantiles" %in% names(nodesplit[[1]])) {
    type <- "dose"
  } else if ("quantiles" %in% names(nodesplit[[1]][[1]])) {
    type <- "time"
  ...
  for (i in seq_along(nodesplit)) {
    ...
    if (type=="dose") {
      ...
      pvals <- c(pvals, rep(nodesplit[[i]]$p.values, 3))
      comp <- c(comp, rep(names(nodesplit)[i], 3))
    } else if (type=="time") {
      for (k in seq_along(nodesplit[[i]])) {
        post <- nodesplit[[i]][[k]]$quantiles
        ...
        pvals <- c(pvals, rep(nodesplit[[i]][[k]]$p.values, 2))
        time.param <- c(time.param, rep(names(nodesplit[[i]])[k], 2))
        comp <- c(comp, rep(names(nodesplit)[i], 2))
      }
    }
  }
...
summary.mbnma <- function(mbnma) {
  ...
  # Print DR function
  overall.sect <- paste("Dose-response function:", mbnma$model.arg$fun, sep=" ")
  overall.sect <- paste(title, overall.sect, sep="\n")

  # Print method section
  method.sect <- print.method.sect(mbnma)

  # Print treatment-level section
  treat.sect <- print.treat.str(mbnma)

  # Class-effect section
  class.sect <- print.class.str(mbnma)
