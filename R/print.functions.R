...
#'
#' @export
summary.mbnma.rank <- function(x, ...) {
  checkmate::assertClass(x, "mbnma.rank")

  output <- list(...)
  for (i in seq_along(x)) {
    output[[names(x)[i]]] <- x[[i]]$summary
  }
  return(output)
}
...
#'
#' @inheritParams summary.mbnma.rank
#'
#' @export
...
#'
#' @inheritParams print.nma.nodesplit
#'
#' @export
summary.nma.nodesplit <- function(x, ...) {
  checkmate::assertClass(x, "nma.nodesplit")

  if ("quantiles" %in% names(x[[1]])) {
    type <- "dose"
  } else if ("quantiles" %in% names(x[[1]][[1]])) {
    type <- "time"
  ...
  for (i in seq_along(x)) {
    ...
    if (type=="dose") {
      ...
      pvals <- c(pvals, rep(x[[i]]$p.values, 3))
      comp <- c(comp, rep(names(x)[i], 3))
    } else if (type=="time") {
      for (k in seq_along(x[[i]])) {
        post <- x[[i]][[k]]$quantiles
        ...
        pvals <- c(pvals, rep(x[[i]][[k]]$p.values, 2))
        time.param <- c(time.param, rep(names(x[[i]])[k], 2))
        comp <- c(comp, rep(names(x)[i], 2))
      }
    }
  }
...
#'
#' @export
summary.mbnma <- function(x, ...) {
  checkmate::assertClass(x, "mbnma")

  # State that function does not work if "parameters.to.save" has been specified
  if (!is.null(x$model.arg$parameters.to.save)) {
  ...
  # Print DR function
  overall.sect <- paste("Dose-response function:", x$model.arg$fun, sep=" ")
  overall.sect <- paste(title, overall.sect, sep="\n")

  # Print method section
  method.sect <- print.method.sect(x)

  # Print treatment-level section
  treat.sect <- print.treat.str(x)

  # Class-effect section
  class.sect <- print.class.str(x)

  # Model fit statistics section
  modfit.sect <- print.modfit.str(x)
