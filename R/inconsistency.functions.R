...
get.relative <- function(lower.diag, upper.diag=lower.diag, treatments=list(),
  ...
  for (mod in seq_along(modlist)) {
    ...
    if ("mbnma" %in% class(modlist[[mod]])) {
      ...
      pred <- suppressMessages(predict(mbnma, exact.doses=treatments,
                      E0=0.2, lim=lim, regress.vals=regress.vals))

      # Convert the named list into a vector of names
      trtnams <- rep(names(treatments), lengths(treatments))
      doses <- unlist(treatments, use.names = FALSE)
