...
#'
#' @export
predict.mbnma <- function(x, n.doses=15, max.doses=NULL, exact.doses=NULL,
                          E0=0, synth="fixed",
                          ...) {
  ######## CHECKS ########
  ...
  agents <- x$model$data()$agent
  mbnma.agents <- x[["agents"]]
  ...
  if (!is.null(doses)) {
    ...
    if (is.null(names(doses))) {
      ...
      if (match.pass==FALSE) {
        stop("Element names in `doses` must correspond either to agent names in data or agent codes in `x`")
      }
    }
    ...
    dose <- as.vector(x$model$data()$dose)
  ...
  link <- x$model.arg$link

  n <- x$BUGSoutput$n.keep * x$BUGSoutput$n.chains

  DR <- suppressMessages(
    write.dose.fun(fun=x$model.arg$fun, user.fun=x$model.arg$user.fun,
                   effect="abs"
                   ))
  DR <- gsub("(^.+<-)(.+)", "\\2", DR)

  betaparams <- get.model.vals(x)
  ...
  if ((is.numeric(E0) | is.character(E0))) {
    ...
    E0 <- ref.synth(data.ab=E0, mbnma=x, synth=synth, ...)
  ...
  output <- list("predicts"=predict.result,
                 "likelihood"=x$model.arg$likelihood, "link"=x$model.arg$link)
