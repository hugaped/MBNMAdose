...
\CommentTok{#> Inference for Bugs model at "C:\textbackslash{}Users\textbackslash{}hp17602\textbackslash{}AppData\textbackslash{}Local\textbackslash{}Temp\textbackslash{}RtmpmeY2V3\textbackslash{}file5fac5a7f2e27", fit using jags,}
...
\CommentTok{#> d[2]        -0.456   0.091   -0.636   -0.516   -0.459   -0.393   -0.278}
\CommentTok{#> d[3]        -0.654   0.045   -0.741   -0.683   -0.655   -0.624   -0.564}
\CommentTok{#> d[4]        -0.710   0.046   -0.796   -0.740   -0.710   -0.681   -0.618}
\CommentTok{#> d[5]        -0.761   0.085   -0.921   -0.820   -0.764   -0.706   -0.587}
\CommentTok{#> d[6]        -0.683   0.168   -1.010   -0.796   -0.685   -0.570   -0.342}
\CommentTok{#> sd           0.124   0.028    0.076    0.105    0.122    0.140    0.186}
\CommentTok{#> totresdev   47.058   9.926   29.304   40.090   46.339   53.643   67.739}
\CommentTok{#> deviance  -124.256   9.926 -142.010 -131.224 -124.975 -117.671 -103.575}
\CommentTok{#>            Rhat n.eff}
\CommentTok{#> d[1]      1.000     1}
\CommentTok{#> d[2]      1.001  3000}
\CommentTok{#> d[3]      1.003   860}
\CommentTok{#> d[4]      1.002  1400}
\CommentTok{#> d[5]      1.001  3000}
\CommentTok{#> d[6]      1.002  1600}
\CommentTok{#> sd        1.003   810}
\CommentTok{#> totresdev 1.001  2600}
...
\CommentTok{#> pD = 49.3 and DIC = -75.0}
...
\CommentTok{#> eletriptan      d.1[2] 2.528909 2.210673 2.913152}
\CommentTok{#> sumatriptan     d.1[3] 1.760485 1.535247 2.045847}
\CommentTok{#> frovatriptan    d.1[4] 1.908290 1.315477 3.315225}
\CommentTok{#> almotriptan     d.1[5] 1.860720 1.396351 3.020261}
\CommentTok{#> zolmitriptan    d.1[6] 2.016668 1.564241 2.644248}
\CommentTok{#> naratriptan     d.1[7] 1.066673 0.583384 2.120677}
\CommentTok{#> rizatriptan     d.1[8] 2.358074 1.884939 3.273184}
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> #### beta.2 (emax, ed50) dose-response parameter results ####}
\CommentTok{#> }
\CommentTok{#> Parameter modelled on exponential scale to ensure it takes positive values}
\CommentTok{#> on the natural scale}
\CommentTok{#> Pooling: relative effects}
\CommentTok{#> }
\CommentTok{#>              Parameter     Median      2.5%      97.5%}
\CommentTok{#> eletriptan      d.2[2] -0.7186570 -1.171510 -0.3051783}
\CommentTok{#> sumatriptan     d.2[3] -0.6493340 -1.202323 -0.1477799}
\CommentTok{#> frovatriptan    d.2[4] -0.5087995 -1.562793  0.5722465}
\CommentTok{#> almotriptan     d.2[5] -0.1934241 -0.970873  0.6874867}
\CommentTok{#> zolmitriptan    d.2[6] -0.4100398 -1.211810  0.1861653}
\CommentTok{#> naratriptan     d.2[7] -0.1665845 -1.208943  1.0159760}
\CommentTok{#> rizatriptan     d.2[8] -0.6278616 -1.363805  0.1182828}
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
...
\CommentTok{#> pD (pV) calculated using the rule, pD = var(deviance)/2 = 85.6}
\CommentTok{#> Deviance = 1169.9}
\CommentTok{#> Residual deviance = 266.8}
\CommentTok{#> Deviance Information Criterion (DIC) = 1255.5}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> Warning in rhat.warning(object): The following parameters have Rhat values > 1.02}
\CommentTok{#> which could be due to convergence issues:}
\CommentTok{#> deviance}
\CommentTok{#> sd}
\CommentTok{#> totresdev}
\CommentTok{#> ========================================}
\CommentTok{#> Dose-response MBNMA}
\CommentTok{#> ========================================}
\CommentTok{#> }
...
\CommentTok{#> -----------------------------------------------------------------------}
\CommentTok{#> Between-study SD for relative effects        0.238 (0.152, 0.333)}
\CommentTok{#> }
\CommentTok{#> }
...
\CommentTok{#> }
\CommentTok{#>              Parameter   Median     2.5%    97.5%}
\CommentTok{#> eletriptan   d.emax[2] 2.812885 2.317632 3.510931}
\CommentTok{#> sumatriptan  d.emax[3] 1.874531 1.604606 2.254731}
\CommentTok{#> frovatriptan d.emax[4] 2.055843 1.396687 2.870282}
\CommentTok{#> almotriptan  d.emax[5] 1.762156 1.352367 2.319707}
\CommentTok{#> zolmitriptan d.emax[6] 2.069016 1.649685 2.625423}
\CommentTok{#> naratriptan  d.emax[7] 1.001239 0.431036 1.664194}
\CommentTok{#> rizatriptan  d.emax[8] 2.668115 2.165375 3.420826}
\CommentTok{#> }
\CommentTok{#> }
...
\CommentTok{#> }
\CommentTok{#>           Parameter     Median       2.5%     97.5%}
\CommentTok{#> beta.ed50 beta.ed50 -0.4081825 -0.9170875 0.1008717}
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
...
\CommentTok{#> }
\CommentTok{#> Effective number of parameters:}
\CommentTok{#> pD (pV) calculated using the rule, pD = var(deviance)/2 = 195}
\CommentTok{#> Deviance = 1095.7}
\CommentTok{#> Residual deviance = 192.6}
\CommentTok{#> Deviance Information Criterion (DIC) = 1290.7}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{noplac.gout <-}\StringTok{ }
\StringTok{  }\NormalTok{GoutSUA_2wkCFB[}\OperatorTok{!}\NormalTok{GoutSUA_2wkCFB}\OperatorTok{$}\NormalTok{studyID }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{2001}\NormalTok{, }\DecValTok{3102}\NormalTok{),] }\CommentTok{# Drop two-arm placebo studies}
\NormalTok{noplac.gout <-}\StringTok{ }
\StringTok{  }\NormalTok{noplac.gout[noplac.gout}\OperatorTok{$}\NormalTok{agent}\OperatorTok{!=}\StringTok{"Plac"}\NormalTok{,] }\CommentTok{# Drop placebo arm from multi-arm studies}
...
\includegraphics{mbnmadose_files/figure-latex/unnamed-chunk-32-1.pdf}
...
\includegraphics{mbnmadose_files/figure-latex/unnamed-chunk-32-2.pdf}
...
\CommentTok{#> Inference for Bugs model at "C:\textbackslash{}Users\textbackslash{}hp17602\textbackslash{}AppData\textbackslash{}Local\textbackslash{}Temp\textbackslash{}RtmpmeY2V3\textbackslash{}file5fac6eec386d", fit using jags,}
\CommentTok{#>  3 chains, each with 10000 iterations (first 5000 discarded), n.thin = 5}
\CommentTok{#>  n.sims = 3000 iterations saved}
\CommentTok{#>            mu.vect sd.vect     2.5%      25%      50%      75%    97.5%}
\CommentTok{#> sd           0.268   0.046    0.181    0.237    0.266    0.300    0.361}
\CommentTok{#> totresdev  189.873  19.143  153.762  176.779  189.445  202.516  228.147}
\CommentTok{#> deviance  1092.979  19.143 1056.868 1079.885 1092.551 1105.622 1131.253}
\CommentTok{#>            Rhat n.eff}
...
\CommentTok{#> }
\CommentTok{#> DIC info (using the rule, pD = var(deviance)/2)}
\CommentTok{#> pD = 183.3 and DIC = 1275.8}
\CommentTok{#> DIC is an estimate of expected predictive error (lower deviance is better).}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> 1   eletriptan 5.396333 1.410408    2   4   6   7     7}
\CommentTok{#> 2  sumatriptan 4.928333 1.530134    2   4   5   6     7}
\CommentTok{#> 3 frovatriptan 4.184333 1.999088    1   3   4   6     7}
\CommentTok{#> 4  almotriptan 2.613000 1.666980    1   1   2   4     7}
\CommentTok{#> 5 zolmitriptan 3.655333 1.795071    1   2   3   5     7}
\CommentTok{#> 6  naratriptan 2.612000 1.883960    1   1   2   4     7}
\CommentTok{#> 7  rizatriptan 4.610667 1.715245    1   3   5   6     7}
\CommentTok{#> }
\CommentTok{#> $d.emax}
\CommentTok{#>     rank.param     mean        sd 2.5% 25% 50% 75% 97.5%}
\CommentTok{#> 1   eletriptan 1.522667 0.7035514    1   1   1   2     3}
\CommentTok{#> 2  sumatriptan 5.096667 0.9217404    3   5   5   6     7}
\CommentTok{#> 3 frovatriptan 4.299333 1.5001610    1   3   4   6     7}
\CommentTok{#> 4  almotriptan 4.518000 1.4265416    1   4   5   6     6}
\CommentTok{#> 5 zolmitriptan 3.674000 1.2034146    1   3   4   4     6}
\CommentTok{#> 6  naratriptan 6.825667 0.6986443    4   7   7   7     7}
\CommentTok{#> 7  rizatriptan 2.063667 0.9479343    1   1   2   3     4}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{mbnmadose_files/figure-latex/unnamed-chunk-40-1.pdf}
...
\includegraphics{mbnmadose_files/figure-latex/unnamed-chunk-40-2.pdf}
...
\CommentTok{#> 1  eletriptan    0 0.1237922 0.003389439 0.1172246 0.1215082 0.1237486}
\CommentTok{#> 2  eletriptan    1 0.4371452 0.018806833 0.3995530 0.4244740 0.4369976}
\CommentTok{#> 3  eletriptan    3 0.5533367 0.026153785 0.5058438 0.5350005 0.5524185}
\CommentTok{#> 4 sumatriptan    0 0.1237922 0.003389439 0.1172246 0.1215082 0.1237486}
\CommentTok{#> 5 sumatriptan    3 0.3866011 0.016821588 0.3550886 0.3749769 0.3864892}
\CommentTok{#>         75%     97.5%}
\CommentTok{#> 1 0.1261082 0.1304652}
\CommentTok{#> 2 0.4502448 0.4735464}
\CommentTok{#> 3 0.5710880 0.6050590}
\CommentTok{#> 4 0.1261082 0.1304652}
\CommentTok{#> 5 0.3977794 0.4194978}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{mbnmadose_files/figure-latex/unnamed-chunk-48-1.pdf}
