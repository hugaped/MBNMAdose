...
\CommentTok{#> Inference for Bugs model at "C:\textbackslash{}Users\textbackslash{}hp17602\textbackslash{}AppData\textbackslash{}Local\textbackslash{}Temp\textbackslash{}RtmpwvhQlZ\textbackslash{}file4c824106817", fit using jags,}
...
\CommentTok{#> d[2]        -0.453   0.089   -0.627   -0.511   -0.456   -0.396   -0.268}
\CommentTok{#> d[3]        -0.654   0.047   -0.746   -0.685   -0.653   -0.623   -0.561}
\CommentTok{#> d[4]        -0.709   0.046   -0.800   -0.739   -0.710   -0.678   -0.615}
\CommentTok{#> d[5]        -0.757   0.086   -0.924   -0.815   -0.757   -0.702   -0.584}
\CommentTok{#> d[6]        -0.677   0.168   -1.004   -0.788   -0.674   -0.566   -0.352}
\CommentTok{#> sd           0.124   0.028    0.075    0.104    0.122    0.141    0.184}
\CommentTok{#> totresdev   46.668  10.044   29.151   39.401   46.049   53.077   68.468}
\CommentTok{#> deviance  -124.646  10.044 -142.163 -131.913 -125.265 -118.237 -102.846}
\CommentTok{#>            Rhat n.eff}
\CommentTok{#> d[1]      1.000     1}
\CommentTok{#> d[2]      1.004   640}
\CommentTok{#> d[3]      1.003   690}
\CommentTok{#> d[4]      1.001  3000}
\CommentTok{#> d[5]      1.002  1300}
\CommentTok{#> d[6]      1.001  3000}
...
\CommentTok{#> pD = 50.5 and DIC = -74.2}
...
\CommentTok{#> }
\CommentTok{#> #### beta.1 dose-response parameter results ####}
\CommentTok{#> Pooling: relative effects }
\CommentTok{#> }
\CommentTok{#> Parameter    Median (95%CrI)}
\CommentTok{#> ---------------------------------}
\CommentTok{#> d.1[2]   2.55 (2.29, 2.91)}
\CommentTok{#> d.1[3]   1.72 (1.5, 1.97)}
\CommentTok{#> d.1[4]   1.82 (1.32, 2.95)}
\CommentTok{#> d.1[5]   1.84 (1.41, 2.75)}
\CommentTok{#> d.1[6]   2.02 (1.63, 2.76)}
\CommentTok{#> d.1[7]   1.03 (0.503, 1.84)}
\CommentTok{#> d.1[8]   2.3 (1.86, 3.12)}
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> #### beta.2 dose-response parameter results ####}
\CommentTok{#> Parameter modelled on exponential scale to ensure it takes positive values on the natural scale}
\CommentTok{#> Pooling: relative effects }
\CommentTok{#> }
\CommentTok{#> Parameter    Median (95%CrI)}
\CommentTok{#> ---------------------------------}
\CommentTok{#> d.2[2]   -0.691 (-1.09, -0.324)}
\CommentTok{#> d.2[3]   -0.74 (-1.26, -0.207)}
\CommentTok{#> d.2[4]   -0.637 (-1.72, 0.457)}
\CommentTok{#> d.2[5]   -0.222 (-0.895, 0.55)}
\CommentTok{#> d.2[6]   -0.44 (-1.14, 0.242)}
\CommentTok{#> d.2[7]   -0.167 (-1.47, 0.835)}
\CommentTok{#> d.2[8]   -0.717 (-1.36, 0.00266)}
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
...
\CommentTok{#> pD (pV) calculated using the rule, pD = var(deviance)/2 = 76}
\CommentTok{#> Deviance = 1170}
\CommentTok{#> Residual deviance = 266}
\CommentTok{#> Deviance Information Criterion (DIC) = 1246}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> Pooling: absolute single parameter }
\CommentTok{#> }
\CommentTok{#> Parameter    Median (95%CrI)}
\CommentTok{#> ---------------------------------}
\CommentTok{#> beta.emax        -1.02 (-1.23, -0.826)}
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> }
\CommentTok{#> #### Pooling method ####}
...
\CommentTok{#> Parameter                    Median (95%CrI)}
\CommentTok{#> -----------------------------------------------------------------------}
\CommentTok{#> Between-study SD for relative effects        0.421 (0.331, 0.546)}
\CommentTok{#> }
\CommentTok{#> #### Class effects ####}
\CommentTok{#> }
\CommentTok{#> Class effect on ed50: common }
\CommentTok{#> }
\CommentTok{#> Parameter    Median (95%CrI)}
\CommentTok{#> ---------------------------------}
\CommentTok{#> D.ed50[2]    -6.91 (-61.6, 2.08)}
\CommentTok{#> D.ed50[3]    -14.6 (-67.6, 5.43)}
\CommentTok{#> D.ed50[4]    7.43 (-24, 66.1)}
\CommentTok{#> }
\CommentTok{#> #### Model Fit Statistics ####}
\CommentTok{#> }
\CommentTok{#> Effective number of parameters:}
\CommentTok{#> pD (pV) calculated using the rule, pD = var(deviance)/2 = 83}
\CommentTok{#> Deviance = -90}
\CommentTok{#> Residual deviance = 77}
\CommentTok{#> Deviance Information Criterion (DIC) = -6}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#>    Observed stochastic nodes: 27}
\CommentTok{#>    Unobserved stochastic nodes: 15}
\CommentTok{#>    Total graph size: 348}
\CommentTok{#> }
...
\CommentTok{#> Error in update.jags(object, n.iter, ...): Error in node y[2,2]}
\CommentTok{#> Failure to calculate log density}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> sd           0.284   0.047    0.180    0.256    0.287    0.315    0.371}
\CommentTok{#> totresdev  185.861  19.398  150.049  172.655  184.975  197.880  227.914}
\CommentTok{#> deviance  1088.967  19.398 1053.155 1075.761 1088.081 1100.986 1131.020}
\CommentTok{#>            Rhat n.eff}
\CommentTok{#> d.1[1,1]  1.000     1}
\CommentTok{#> d.1[1,2]  1.014   340}
\CommentTok{#> d.1[2,2]  1.021   120}
\CommentTok{#> d.1[3,2]  1.006   410}
\CommentTok{#> d.1[1,3]  1.020   120}
\CommentTok{#> d.1[2,3]  1.083    29}
\CommentTok{#> d.1[3,3]  1.023   110}
\CommentTok{#> d.1[4,3]  1.080    30}
\CommentTok{#> d.1[1,4]  1.010   250}
\CommentTok{#> d.1[2,4]  1.013   160}
\CommentTok{#> d.1[1,5]  1.020  1300}
\CommentTok{#> d.1[2,5]  1.012   210}
\CommentTok{#> d.1[3,5]  1.007   320}
\CommentTok{#> d.1[1,6]  1.005  2200}
\CommentTok{#> d.1[2,6]  1.030    80}
\CommentTok{#> d.1[3,6]  1.024    96}
\CommentTok{#> d.1[4,6]  1.014   210}
\CommentTok{#> d.1[5,6]  1.070    35}
\CommentTok{#> d.1[1,7]  1.019   110}
\CommentTok{#> d.1[2,7]  1.005   470}
\CommentTok{#> d.1[1,8]  1.004  1200}
\CommentTok{#> d.1[2,8]  1.011   210}
\CommentTok{#> d.1[3,8]  1.055    43}
\CommentTok{#> sd        1.055   150}
\CommentTok{#> totresdev 1.003  3000}
\CommentTok{#> deviance  1.004  2700}
\CommentTok{#> }
\CommentTok{#> For each parameter, n.eff is a crude measure of effective sample size,}
\CommentTok{#> and Rhat is the potential scale reduction factor (at convergence, Rhat=1).}
\CommentTok{#> }
\CommentTok{#> DIC info (using the rule, pD = var(deviance)/2)}
\CommentTok{#> pD = 188.1 and DIC = 1277.1}
\CommentTok{#> DIC is an estimate of expected predictive error (lower deviance is better).}
\end{Highlighting}
\end{Shaded}
...
cannot be performed on non-parameteric models at the moment, as these
...
\CommentTok{#> 1   eletriptan 5.349667 1.250302    3   5   5   6     7}
\CommentTok{#> 2  sumatriptan 5.073667 1.572598    2   4   5   6     7}
\CommentTok{#> 3 frovatriptan 4.309000 1.919917    1   3   4   6     7}
\CommentTok{#> 4  almotriptan 2.488667 1.596680    1   1   2   3     7}
\CommentTok{#> 5 zolmitriptan 3.592667 1.724839    1   2   3   5     7}
\CommentTok{#> 6  naratriptan 2.498333 1.843673    1   1   2   3     7}
\CommentTok{#> 7  rizatriptan 4.688000 1.764188    1   3   5   6     7}
\CommentTok{#> }
\CommentTok{#> $d.emax}
\CommentTok{#>     rank.param     mean        sd 2.5% 25% 50% 75% 97.5%}
\CommentTok{#> 1   eletriptan 1.520333 0.6253401    1   1   1   2     3}
\CommentTok{#> 2  sumatriptan 5.186000 0.9253951    3   5   5   6     7}
\CommentTok{#> 3 frovatriptan 4.388667 1.4597883    1   3   4   6     7}
\CommentTok{#> 4  almotriptan 4.289667 1.4513656    1   3   4   5     6}
\CommentTok{#> 5 zolmitriptan 3.609333 1.2529576    1   3   3   4     6}
\CommentTok{#> 6  naratriptan 6.771667 0.8655135    3   7   7   7     7}
\CommentTok{#> 7  rizatriptan 2.234333 1.2068858    1   1   2   3     5}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> 1  eletriptan    0 0.1240604 0.003308866 0.1175943 0.1218852 0.1240392}
\CommentTok{#> 2  eletriptan    1 0.4361478 0.019118415 0.3997727 0.4229120 0.4358769}
\CommentTok{#> 3  eletriptan    3 0.5549116 0.027408683 0.5054910 0.5345451 0.5542398}
\CommentTok{#> 4 sumatriptan    0 0.1240604 0.003308866 0.1175943 0.1218852 0.1240392}
\CommentTok{#> 5 sumatriptan    3 0.3867294 0.017660379 0.3512009 0.3747711 0.3865771}
\CommentTok{#>         75%     97.5%}
\CommentTok{#> 1 0.1263792 0.1304690}
\CommentTok{#> 2 0.4492309 0.4744319}
\CommentTok{#> 3 0.5729344 0.6109603}
\CommentTok{#> 4 0.1263792 0.1304690}
\CommentTok{#> 5 0.3984916 0.4220573}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> ========================================}
\CommentTok{#> Node-splitting analysis of inconsistency}
\CommentTok{#> ========================================}
\CommentTok{#>  comparison  p.value         Median (95% CrI)}
\CommentTok{#> 12v7     0.1636}
\CommentTok{#> -> direct                0.288 (-0.282, 0.857)}
\CommentTok{#> -> indirect              0.904 (0.324, 1.49)}
\CommentTok{#> -> NMA                   0.213 (-0.208, 0.66)}
\CommentTok{#> }
\CommentTok{#> 4v3      0.3939}
\CommentTok{#> -> direct                0.122 (-0.536, 0.763)}
\CommentTok{#> -> indirect              0.35 (0.0402, 0.668)}
\CommentTok{#> -> NMA                   0.216 (-0.125, 0.547)}
\CommentTok{#> }
\CommentTok{#> 5v2      0.2407}
\CommentTok{#> -> direct                0.184 (-0.466, 0.781)}
\CommentTok{#> -> indirect              0.559 (0.272, 0.852)}
\CommentTok{#> -> NMA                   0.498 (0.251, 0.765)}
\CommentTok{#> }
\CommentTok{#> 3v2      0.2611}
\CommentTok{#> -> direct                -0.403 (-1.04, 0.213)}
\CommentTok{#> -> indirect              -0.000421 (-0.33, 0.328)}
\CommentTok{#> -> NMA                   -0.0896 (-0.374, 0.207)}
\end{Highlighting}
\end{Shaded}
