...
* Spline functions (`dspline()`): B-splines (`type="bs"`), natural splines (`type="ns"`), restricted cubic splines (`type="rcs"`) and piecewise linear splines (`type="ls"`) can be fitted. $f(x_{i,k}, t_{i,k})=\sum_{p=1}^{P} \beta_{p,t_{i,k}} X_{p,i,k}$ where $\beta_{p,t_{i,k}}$ is the regression coefficient for the $p^{th}$ spline and $X_{1:P,i,k}$ is the basis matrix for the spline, defined by the spline type.
...
With some dose-response functions (e.g. splines, fractional polynomials) parameter interpretation can be challenging. The `get.relative()` function can make this easier as this allows relative effects to be estimated between agents at specific doses, which is typically much more easily understandable and presentable.
...
get.relative(mbnma, treatments = treats)
```
...
dr.funs <- dmulti(list(
  ...
  "fluoxetine"=dspline(type="rcs",knots=3),
  "paroxetine"=dpoly(degree=2),
  "sertraline"=dspline(type="rcs",knots=3)
  ))
...
# Using a restricted cubic spline on the SSRI dataset
depnet <- mbnma.network(ssri)
rcs <- mbnma.run(depnet, fun=dspline(type="rcs", knots=c(0.25,0.5,0.75)))
```
