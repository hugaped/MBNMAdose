...
-   Log-linear (`dloglin()`):
    $f(x_{i,k}, t_{i,k})=\lambda_{t_{i,k}} \times ln(x_{i,k} + 1)$ where
    $lambda$ controls the gradient of the dose-response relationship.

-   Exponential (`dexp()`):
    $f(x_{i,k}, t_{i,k})=Emax_{t_{i,k}} (1 - e^{-x_{i,k}})$ where $Emax$
...
-   Emax (`demax()`):
    ...
    the dose at which 50% of the maximum efficacy is achieved, and
    $\gamma$ is the Hill parameter that controls the sigmoidal shape of
    the function. By default, `demax()` fits a 2-parameter Emax function
...
-   Polynomial (e.g. linear) (`dpoly()`):
    ...
    where $p$ is the degree of the polynomial (e.g. 1 for linear) and
    $\beta_p$ are the coefficients

-   Fractional polynomial (`dfpoly()`):
    $f(x_{i,k}, t_{i,k})=\beta_{1_{t_{i,k}}}x_{i,k}^{\gamma_1}+...+\beta_{p_{t_{i,k}}}x^{\gamma_p}_{i,k}$
    where $p$ is the degree of the polynomial, $\beta_p$ are the
    coefficients, and $x_{i,k}^{\gamma_p}$ is a regular power except
    where $\gamma_p=0$ where ${x_{i,k}^{(0)}=ln(x_{i,k})}$. If a
    fractional polynomial power ${\gamma_p}$ repeats within the function
    it is multiplied by another ${ln(x_{i,k})}$.

-   Spline functions (`dspline()`): B-splines (`type="bs"`), natural
    cubic splines (`type="ns"`) and piecewise linear splines
    (`type="ls"`) can be fitted.
    $f(x_{i,k}, t_{i,k})=\sum_{p=1}^{P} \beta_{p,t_{i,k}} X_{p,i,k}$
    where $\beta_{p,t_{i,k}}$ is the regression coefficient for the
    $p^{th}$ spline and $X_{1:P,i,k}$ is the basis matrix for the
    spline, defined by the spline type.

-   Non-parametric monotonic function (`dnonparam()`): Follows the
    approach of Owen et al. [-@owen2015]. The `direction` can be
    specified as `"increasing"` or `"decreasing"`.

-   User-defined function (`duser()`): Any function that can be
    explicitly defined by the user within (see [User-defined
    dose-response function])

-   Agent-specific functions (`dmulti()`): Allows for a separate
    dose-response function to be fitted to each agent in the network
    (see [Agent-specific dose-response functions])
...
`link="smd"`. Currently, `MBNMAdose` standardises treatment effects
using the pooled standard deviation (SD) in each study. The resulting
treatment effects are reported in units of SD as SMDs.

A more robust approach to minimise bias from estimation of within-study
SD would be to use a single reference SD for standardisation of each
scale included in the dataset. This is something `MBNMAdose` will
incorporate in the future. For further details of analysis of continuous
data that include discussion of both RoM and SMD see [@GMD2].
...
[-@thorlund2015]), though we expect that this may lead to bias if the
...
found in the vignette on [NMA in `MBNMAdose`](mbnmadose.html)
...
relationships, though parameters can be difficult to interpret and the resulting dose-response shape is often best visualised by calculating and plotting predictions  [Prediction](mbnmadose.html).
...
range for each agent at which knots should be located.
...
TO ADD - USE SSRI DATASET WITH FRACTIONAL POLYNOMIAL AS EXAMPLE AND
BINARY BIAS - first convert bias to numeric and use mod risk as
reference category - then use mbnma.run(ssrinet, fun=fpoly(degree=2),
regress.vars=c("rob")) - Show with common (no evidence of regress),
agent (evidence on agent 4 - fluoxetine) - show plots - Highlight risk
of aggregation bias with non-identity link functions (Phillippo)

Note aggregation bias when modelling using non-identity link functions
(refer to David's paper/thesis) Mention about centring variables, only
taking binary or continuous (means that need to generate dummy variables
from factors - can be done using model.matrix())

One could envision a more complex model in which the interaction effect
also varied by a dose-response relationship, rather than assuming an
effect by agent/class or across the whole network. This would in
principle contain fewer parameters than a fully independent interaction
model (in which a separate regression covariate is estimated for each
treatment in the dataset).

#### Correlation between dose-response parameters

`mbnma.run()` automatically models correlation between relative effects
dose-response parameters. This can be prevented by specifying
`cor=FALSE` in `mbnma.run()`. The correlation is modeled using a vague
...
standard NMA. Relative effects can take the form of odds ratios, mean
...
Predicted responses can also be plotted using the `plot()` method on an
...
`MBNMAdose` provides a complete set of functions that allow for
