...
...where $\phi$ is an identifier for the dose-response parameter, $a$ is an agent identifier and $c$ is a class identifier
...
After performing an MBNMA, responses can be predicted from the model parameter estimates using `predict()` on an `"MBNMA"` object. A number of important arguments *must* be specified for prediction.
...
If there have been predictions at dose = 0 **for each agent** only one of these will be included in the rankings, in order to avoid duplication (since the predicted response at dose = 0 is the same for all agents).
